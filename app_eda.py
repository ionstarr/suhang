...
class EDA:
    def __init__(self):
        ...
        # 9. 인구 데이터 분석
        with tabs[8]:
            st.header("🌍 인구 데이터 분석 (population_trends.csv)")
            ...
            st.success("✅ population_trends.csv 파일 로드 완료!")
            ...
            # 공통: 결측치/중복 제거
            with pop_tabs[0]:
                st.subheader("📋 기초 통계 및 품질 점검")
                st.write(f"총 행 수: {pop_df.shape[0]}, 총 열 수: {pop_df.shape[1]}")
                st.write("결측치 개수:")
            ...
            with pop_tabs[1]:
                ...
                if '연도' not in pop_df.columns:
                    st.error("'연도' 컬럼이 존재하지 않습니다.")
                else:
                    year_df = pop_df.groupby('연도')['총인구수'].sum().reset_index()
                    fig, ax = plt.subplots()
                    sns.lineplot(x='연도', y='총인구수', data=year_df, marker='o', ax=ax)
                    ax.set_ylabel("총인구수")
                    ax.set_title("연도별 총 인구 추이")
                    st.pyplot(fig)

            # 지역별 변화량 분석
            with pop_tabs[2]:
                st.subheader("📌 지역별 인구 변화량")
                if not {'지역', '연도', '총인구수'}.issubset(pop_df.columns):
                    st.error("필수 컬럼(지역, 연도, 총인구수)이 누락되었습니다.")
                else:
                    pivot = pop_df.pivot(index='연도', columns='지역', values='총인구수')
                    delta_df = pivot.diff().iloc[1:]  # 변화량
                    latest_change = delta_df.iloc[-1].sort_values(ascending=False)
                    st.dataframe(latest_change.to_frame(name='최근 1년 변화량').style.background_gradient(cmap='coolwarm'))

            # 증감률 상위 지역 및 연도
            with pop_tabs[3]:
                st.subheader("📊 연도별 지역 증감률 상위")
                pivot = pop_df.pivot(index='연도', columns='지역', values='총인구수')
                pct_change = pivot.pct_change().iloc[1:] * 100
                melted = pct_change.reset_index().melt(id_vars='연도', var_name='지역', value_name='증감률(%)')
                top_changes = melted.sort_values('증감률(%)', ascending=False).head(10)
...
Page_Home     = st.Page(lambda: Home(Page_Login, Page_Register, Page_FindPW, Page_EDA), title="Home", icon="🏠", url_path="home", default=True)
